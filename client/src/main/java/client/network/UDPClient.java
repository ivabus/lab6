...
public class UDPClient {
  ...
  private void sendData(byte[] data) throws IOException {
    ...
    logger.info("Отправляется " + ret.length + " блоков...");
    ...
    for(int i = 0; i < ret.length; i++) {
      ...
      if (i == ret.length - 1) {
        ...
        logger.info("Последний блок размером " + lastChunk.length + " отправлен на сервер.");
      } else {
        var answer = Bytes.concat(chunk, new byte[]{0});
        client.send(ByteBuffer.wrap(answer), addr);
        logger.info("Блок размером " + answer.length + " отправлен на сервер.");
      }
    }
